...
class AddItemButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: AlchemyChamberView = self.view
        if interaction.user == view.get_user():
            response = view.add_alchemizeing_item()
            await interaction.response.edit_message(content=None, embed=response, view=view)
...
class RemoveItemButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: AlchemyChamberView = self.view
        if interaction.user == view.get_user():
            response = view.remove_alchemizeing_item()
            await interaction.response.edit_message(content=None, embed=response, view=view)


class SelectAlchemizeingIngredientButton(discord.ui.Button):
    def __init__(self, item_index: int, item: Item, row: int):
        super().__init__(style=discord.ButtonStyle.secondary, label=f"{item.get_name_and_count()}", row=row, emoji=item.get_icon())
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: AlchemyChamberView = self.view
        if interaction.user == view.get_user():
            response = view.select_alchemizeing_ingredient(self._item_index, self._item)
            await interaction.response.edit_message(content=None, embed=response, view=view)
...
class ConfirmAlchemizeingButton(discord.ui.Button):
    def __init__(self, row: int):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: AlchemyChamberView = self.view
        if interaction.user == view.get_user():
            response = view.try_alchemizeing()
            await interaction.response.edit_message(content=None, embed=response, view=view)
...
class AlchemyChamberView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, user: discord.User, house_view: HouseView):
        ...
        self._current_alchemizeing: Dict[ItemKey, int] = {}
    ...
    def get_embed_for_intent(self, error: str=""):
        ...
        if self._intent == Intent.Alchemize:
            current_alchemizeing_str = self.get_current_alchemizeing_str()
            current_alchemizeing_display = f"──────────\n{current_alchemizeing_str}──────────\n\n" if current_alchemizeing_str != "" else ""
    ...
    def get_current_alchemizeing_str(self):
        input_strs = []
        for item_key, quantity in self._current_alchemizeing.items():
            if quantity > 0:
                item_data = LOADED_ITEMS.get_item_state(item_key)
                input_strs.append(f"{item_data['icon']} {item_data['name']} (x{quantity})\n")
        return '\n'.join(input_strs)

    def add_alchemizeing_item(self):
        player: Player = self._get_player()
        inventory_slots: List[Item] = player.get_inventory().get_inventory_slots()
        if self._selected_item is None or inventory_slots[self._selected_item_index] != self._selected_item:
            return self.get_embed_for_intent(error="\n\n*Error: Something about that item changed or it's no longer available.*")
        ...
        self._current_alchemizeing[item_key] = self._current_alchemizeing.get(item_key, 0) + 1

        current_alchemizeing_str = self.get_current_alchemizeing_str()
        current_alchemizeing_display = f"──────────\n{current_alchemizeing_str}──────────\n\n" if current_alchemizeing_str != "" else ""
        return Embed(title="Alchemize", description=f"{current_alchemizeing_display}Mix together ingredients from your inventory and attempt to create a potion.\n\nNavigate through the items using the Prev and Next buttons.")

    def remove_alchemizeing_item(self):
        player: Player = self._get_player()
        inventory_slots: List[Item] = player.get_inventory().get_inventory_slots()
        if self._selected_item is None or inventory_slots[self._selected_item_index] != self._selected_item:
            return self.get_embed_for_intent(error="\n\n*Error: Something about that item changed or it's no longer available.*")
        ...
        item_key = self._selected_item.get_key()
        self._current_alchemizeing[item_key] = max(self._current_alchemizeing.get(item_key, 0) - 1, 0)

        current_alchemizeing_str = self.get_current_alchemizeing_str()
        current_alchemizeing_display = f"──────────\n{current_alchemizeing_str}──────────\n\n" if current_alchemizeing_str != "" else ""
        return Embed(title="Alchemize", description=f"{current_alchemizeing_display}Mix together ingredients from your inventory and attempt to create a potion.\n\nNavigate through the items using the Prev and Next buttons.")

    def select_alchemizeing_ingredient(self, index: int, item: Item):
        self._selected_item = item
        self._selected_item_index = index
    ...
    def use_recipe(self):
        ...
        return Embed(title="Recipes", description=f"Alchemizeing successful! You received:\n\n{output_display}\n{xp_display}\n\nChoose a recipe you've acquired or discovered to make.\n\nNavigate through your recipes using the Prev and Next buttons.")

    def try_alchemizeing(self):
        if len(self._current_alchemizeing.items()) == 0:
        ...
        # Validation before continuing, slightly slower but avoids having to remove and then return items when something goes wrong
        for input_key, quantity in self._current_alchemizeing.items():
            index = inventory.search_by_key(input_key)
            if index == -1:
        ...
        for recipe_key in LOADED_RECIPES.get_all_keys():
            ...
            if recipe.inputs == self._current_alchemizeing:
        ...
        return Embed(title="Alchemize", description=f"Alchemizeing successful! You received:\n\n{output_display}\n{xp_display}{new_recipe_str}\n──────────\n\nChoose a recipe you've acquired or discovered to make.\n\nNavigate through your recipes using the Prev and Next buttons.")
