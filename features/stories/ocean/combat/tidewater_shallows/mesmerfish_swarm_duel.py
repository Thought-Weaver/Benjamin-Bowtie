...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Break Control", description=f"With the mesmerfish swarm vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: MesmerfishSwarmDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [Mesmerfish(), Mesmerfish(), Mesmerfish()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class MesmerfishSwarmDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Look at It", description="Among the beautiful schools of fish that dart and swim about the various underwater plants and rocky structures of the ocean floor, one of the groups stands out: They move more elegantly, trailing barbed tendrils that look razor sharp.\n\nSuddenly, they turn towards your party and you can see two... no, three... no, more eyes peering from flaps of reddish-pink scaly flesh, split into an 8-part radial parting. You should get closer. Yes, go to the fish. It's incredibly important you continue to stare at this fish --")
