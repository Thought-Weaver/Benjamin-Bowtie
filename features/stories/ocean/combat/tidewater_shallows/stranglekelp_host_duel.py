...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Below the Kelp", description=f"With the stranglekelp host vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: StranglekelpHostDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [StranglekelpHost()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class StranglekelpHostDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="A Grim Fate", description="Though you'd heard Yenna's warning about stranglekelp, nothing quite puts it into perspective more than seeing the shambling figure coming towards you: Rusted diving gear covers its body and in its hand is a rusted harpoon, leaving only the rough figure of a human being. Protruding from cracks and crevices across its now-fragile armor are clumps of stranglekelp, swaying lightly -- then suddenly faster as the creature begins to charge!")
