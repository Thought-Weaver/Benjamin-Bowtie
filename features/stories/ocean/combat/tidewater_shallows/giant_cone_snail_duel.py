...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Slowly Undone", description=f"With the giant cone snail vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: GiantConeSnailDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [GiantConeSnail()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class GiantConeSnailDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="A Mesmerizing Shell", description="Though you've never seen one this large, as you continue on your path, slowly moving ahead towards your party is an enormous creature: Its shell shifts with a triangular pattern that almost seems hypnotizing and its eye stalks move around in the water with a clear, dangerous intent.")
