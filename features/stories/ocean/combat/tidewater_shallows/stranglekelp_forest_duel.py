...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Cut Down", description=f"With the stranglekelp forest traversed, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: StranglekelpForestDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [StranglekelpHoldfast(), StranglekelpHoldfast()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class StranglekelpForestDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Verdant Reaches", description="One of the most incredible sights in this region now stands before you: A wide forest of verdant vines and pointed leaves, stretching up towards the light glinting off the surface of the waves.\n\nThis otherwise serene landscape is starkly contrasted by what lies at the base of these plants: hordes of bones from various creatures. At once, it becomes apparent this is no ordinary seaweed grove, but a forest of stranglekelp you have no choice but to cross.")
