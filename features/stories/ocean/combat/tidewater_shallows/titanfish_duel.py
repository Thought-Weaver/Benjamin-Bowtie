...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Attack On", description=f"With the titanfish vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: TitanfishDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [Titanfish()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class TitanfishDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Titanic Might", description="It's hard to miss the sheer size and unusual shape of the fish coming towards you -- it looks like the head of a fish without a tail, flattened vertically, with two large eyes on either side. Its powerful grey fins and large maw present an imposing sight -- and for some reason this one doesn't seem neutral as the others you've passed so far have been.")
