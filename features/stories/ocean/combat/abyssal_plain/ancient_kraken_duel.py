...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Inkredible Victory", description=f"With the ancient kraken vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: AncientKrakenDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [AncientKraken()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class AncientKrakenDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Mounting Dread", description="There are few things in the ocean that people who've spent their entire lives on land know about -- but the creature you see propelling itself through the murky depths is certainly one of them. Giant tentacles as long as a warship, reddish-orange skin a mark of its fury, and unblinking beady eyes betray a truly dangerous creature: the kraken.")
