...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Still Twitching", description=f"With the wriggling mass vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: WrigglingMassDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [WrigglingMass()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class WrigglingMassDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Wriggle Wriggle", description="The darkness of the abyss makes it difficult to see anything more than a few meters ahead of you, but a sudden movement kicking up the sand puts you on edge.\n\nSomething is thrashing and struggling nearby -- then suddenly there's a piercing noise in your mind, like the aftershock of an explosion and the roar of ocean waves. A ball formed from hundreds of tentacles and eyes rages towards you!")
