...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="The Light Fades", description=f"With the voidseen angler vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: VoidseenAnglerDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [VoidseenAngler()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class VoidseenAnglerDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="A Light in the Dark", description="Seeing a light this far below the surface is a surprising thing, even more so to see a light that's moving. Could it be another person? Perhaps it's worth investigating -- go closer towards the light. Don't mind the teeth. No, I'm just happy to see you! Come here, come here.")
