...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Not So Great", description=f"With the grand lionfish vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: GrandLionfishDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [GrandLionfish()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class GrandLionfishDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Unmatched Grandeur", description="There are few sights as imposing or majestic as the giant fish in this coral forest -- one in particular catches your eye as it begins to descend towards your party! An array of spines on its dorsal fins, red and black banding on its body, and elongated ribbon-like appendages ripple out from a 10 foot tall lionfish seeking its next meal.")
