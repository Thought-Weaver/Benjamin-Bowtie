...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Band Together!", description=f"With the banded eel vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: BandedEelDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [BandedEel()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class BandedEelDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Perforations in Stone", description="Continuing through the coral forest, you're suddenly caught off guard by a sharp bite at your leg -- but when you look down, nothing's there. All that exists to your right is a rather large boulder with some yellow and red corals glinting in the sunlight.\n\nThough, perhaps a bit odd, this boulder seems to have some moderately sized holes in it. You get closer to take a look, when leaping at your face comes an eel with razor teeth!")
