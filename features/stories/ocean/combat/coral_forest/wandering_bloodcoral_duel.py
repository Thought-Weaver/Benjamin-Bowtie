...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Silent and Still", description=f"With the wandering bloodcoral vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: WanderingBloodcoralDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [WanderingBloodcoral()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class WanderingBloodcoralDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Crimson Coral", description="Most of the coral in this forest is a beautiful sight: Golds, blues, and purples play out in various shapes from tubes to latticed networks to shelves jutting from large boulders.\n\nBut, the thing in front of you is like a nightmare; this coral is a deep crimson with various bones from small fish, sharks, and other creatures now beyond identification. Pustules mark its body, like sacs of blood growing on what appears almost like a quadrupedal body. What's even worse is that it's moving quickly -- right towards you!")
