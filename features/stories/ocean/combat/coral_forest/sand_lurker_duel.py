...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Back to the Sands", description=f"With the sand lurker vanquished, the rest of the ocean awaits.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [SandLurker()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class SandLurkerDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Shifting in the Sand", description="Predators in this area have been numerous, but most are easy to spot if you're paying attention: They'll dive around the coral from above or be large enough that it'd be complete folly not to see them.\n\nThe shark-like creature that bursts from below the ground, therefore, is quite a surprise: Erupting from the sand, gnashing with multiple rows of teeth, comes a monster of the deep!")
