...
class RoomButton(discord.ui.Button):
    def __init__(self, icon: str, next_view: discord.ui.View, room_type: RoomType, index: int):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: UnderworldRoomSelectionView = self.view
        ...
        if self._room_type == RoomType.Combat:
            view.get_dungeon_run().previous_combat = self._index
            ...
            view.get_dungeon_run().previous_event = self._index
...
class UnderworldRoomSelectionView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        ...
        self.setup_underworld_rooms()

    def _get_player(self, user_id: int) -> Player:
        return self._database[str(self._guild_id)]["members"][str(user_id)]
    
    def setup_underworld_rooms(self):
        if self._dungeon_run.rooms_until_boss == -1:
            room = underworld.UnderworldStory.generate_boss_room(self._bot, self._database, self._guild_id, self._users, self._dungeon_run)
            self.add_item(RoomButton("\u2620\uFE0F", room, RoomType.Boss, -1))
            return

        if self._dungeon_run.rooms_until_boss == 0:
            room = underworld.UnderworldStory.generate_rest_room(self._bot, self._database, self._guild_id, self._users, self._dungeon_run)
            self.add_item(RoomButton("\uD83D\uDD25", room, RoomType.Rest, -1))
            return
        ...
        for _ in range(num_rooms):
            ...
            if room_rand_gen < SHOPKEEP_ROOM_PROB:
                ...
                if self._dungeon_run.dungeon_type == Story.Underworld:
                    room = underworld.UnderworldStory.generate_shopkeep_room(self._bot, self._database, self._guild_id, self._users, self._dungeon_run)
                    self.add_item(RoomButton(icon, room, RoomType.Shopkeep, -1))
                ...
                if self._dungeon_run.dungeon_type == Story.Underworld:
                    room = underworld.UnderworldStory.generate_rest_room(self._bot, self._database, self._guild_id, self._users, self._dungeon_run)
                    self.add_item(RoomButton(icon, room, RoomType.Rest, -1))
                ...
                if mystery_room_rand_gen < MYSTERY_TREASURE_BASE_PROB + MYSTERY_TREASURE_PROB_INCREASE * self._dungeon_run.num_mystery_without_treasure:
                    ...
                    if self._dungeon_run.dungeon_type == Story.Underworld:
                        room = underworld.UnderworldStory.generate_treasure_room(self._bot, self._database, self._guild_id, self._users, self._dungeon_run)
                        self.add_item(RoomButton(icon, room, RoomType.Treasure, -1))
                    ...
                    if self._dungeon_run.dungeon_type == Story.Underworld:
                        room, index = underworld.UnderworldStory.generate_combat_room(self._bot, self._database, self._guild_id, self._users, self._dungeon_run)
                        self.add_item(RoomButton(icon, room, RoomType.Combat, index))
                else:
                    self._dungeon_run.num_mystery_without_combat += 1
                    self._dungeon_run.num_mystery_without_treasure += 1
                    self._dungeon_run.num_mystery_without_shopkeep += 1
                    ...
                    if self._dungeon_run.dungeon_type == Story.Underworld:
                        room, index = underworld.UnderworldStory.generate_event_room(self._bot, self._database, self._guild_id, self._users, self._dungeon_run)
                        self.add_item(RoomButton(icon, room, RoomType.Event, index))
            else:
                icon = "\u2694\uFE0F"
