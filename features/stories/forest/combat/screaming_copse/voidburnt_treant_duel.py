...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Just Ashes", description=f"With the voidburnt treant vanquished, the rest of the woods await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: VoidburntTreantDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [VoidburntTreant()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class VoidburntTreantDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Bellowing Flame", description="Unlike most of the trees you've seen so far, one ahead on the path catches your attention: It remains lit aflame, burning with a dark purplish fire that seems to shimmer like the night sky. As you move towards it, the stars seem to move as though the fire itself was a window to the void. As you reach it, the tree begins to come alive and roars with a mad fury!")
