...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="In More Dire Straits", description=f"With the starving dire wolves vanquished, the rest of the woods await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: StarvingDireWolvesDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [StarvingDireWolf(" 1"), StarvingDireWolf(" 2")], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class StarvingDireWolvesDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="A Hungry Growl", description="From behind the fallen trees and dead brush, two emaciated figures emerge, eyes glowing yellow: dire wolves!")
