...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="A Howl of Victory", description=f"With the timberwolves vanquished, the rest of the woods await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: TimberwolvesDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [Timberwolf(" 1"), Timberwolf(" 2"), Timberwolf(" 3"), Timberwolf(" 4")], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class TimberwolvesDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Eyes in the Dark", description="In the depths of the woods, you find yourselves surrounded on all sides by wolves!")
