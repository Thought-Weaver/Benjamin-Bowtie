...
from features.stories.forest.combat.npcs.burly_treant import BurlyTreant
from features.stories.forest.combat.npcs.lithe_treant import LitheTreant
...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Chopped Wood", description=f"With the treants vanquished, the rest of the woods await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: TreantsDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [LitheTreant(), BurlyTreant()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class TreantsDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="A Couple of Trees?", description="Directly rooted on the path, one large tree and one smaller one block the way forward. You're about to go around them when they suddenly begin to shift and move -- as they turn to face you, the party gazes upon the three hollow eyes and gaping maw of two dangerous treants!")
