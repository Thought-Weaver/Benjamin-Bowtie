...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: QuietGroveFishingPondView = self.view
...
class QuietGroveFishingPondView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="A Small Pond", description=f"Just near the path lies a little pond and an opportunity to fish!\n\n{len(self.fished_users)}/{len(self._users)} have fished.")
    ...
    def fish(self, user: discord.User):
        ...
        # 37.5% chance of getting a Common fish reward
        if rand_val == 0:
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.BrownTench), 
                LOADED_ITEMS.get_new_item(ItemKey.PondGuppy)
            ]
            fishing_result = random.choice(items)
            xp_to_add = 3
            player_stats.fish.common_fish_caught += 1
        # 27.5% chance of getting an Uncommon fish reward
        elif rand_val == 1:
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.Algaefish),
                LOADED_ITEMS.get_new_item(ItemKey.Jumpfish)
            ]
            fishing_result = random.choice(items)
            xp_to_add = 5
            player_stats.fish.uncommon_fish_caught += 1
        # 17.5% chance of getting a Rare fish reward
        elif rand_val == 2:
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.Blackscale),
                LOADED_ITEMS.get_new_item(ItemKey.GoldenTench)
            ]
            fishing_result = random.choice(items)
            xp_to_add = 8
            player_stats.fish.rare_fish_caught += 1
        # 12.5% chance of getting an Epic fish reward
        elif rand_val == 3:
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.PondEel),
                LOADED_ITEMS.get_new_item(ItemKey.RubyDartfish)
            ]
            fishing_result = random.choice(items)
            xp_to_add = 13
            player_stats.fish.epic_fish_caught += 1
        # 5% chance of getting a Legendary fish reward
        else:
            items = [
