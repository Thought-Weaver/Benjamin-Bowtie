...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: MistyTunnelsUndergroundLakeView = self.view
...
class MistyTunnelsUndergroundLakeView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Underground Lake", description=f"An offshoot of one of the tunnels leads into a large opening where you spot a moderately-sized underground lake. Based on the bubbles, you suspect that some underground fish make their homes here and you could fish for supplies.\n\n{len(self.fished_users)}/{len(self._users)} have fished.")
    ...
    def fish(self, user: discord.User):
        ...
        rand_val = random.choices(
            [0, 1, 2, 3, 4], k=1,
            weights=[
                0.375 - LUCK_MOD * total_luck,
                0.275 + 6 * (LUCK_MOD * total_luck) / 16,
                0.175 + 4 * (LUCK_MOD * total_luck) / 16,
                0.125 + 3 * (LUCK_MOD * total_luck) / 16,
        ...
        if rand_val == 0:
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.Snakehead), 
                LOADED_ITEMS.get_new_item(ItemKey.BrownBrotula)
            ]
            ...
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.DeepEel),
                LOADED_ITEMS.get_new_item(ItemKey.Mudfish)
            ]
            fishing_result = random.choice(items)
            xp_to_add = 5
            ...
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.Knifefish),
                LOADED_ITEMS.get_new_item(ItemKey.PaleTetra)
            ]
            fishing_result = random.choice(items)
            xp_to_add = 8
            ...
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.Glassfish),
                LOADED_ITEMS.get_new_item(ItemKey.AshenCavefish)
            ]
            fishing_result = random.choice(items)
            xp_to_add = 13
            ...
            items = [
                LOADED_ITEMS.get_new_item(ItemKey.TheBigGulper),
                LOADED_ITEMS.get_new_item(ItemKey.StalagmiteFish)
            ]
            fishing_result = random.choice(items)
            xp_to_add = 21
