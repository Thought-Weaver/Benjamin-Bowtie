...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Disengage!", description=f"With the warping anomaly vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: WarpingAnomalyDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [WarpingAnomaly()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class WarpingAnomalyDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Engage!", description="Looking ahead, something about the corridor isn't quite right: The stone almost looks like it's rotating into itself, everything being distorted into a strange shape. Then, unexpectedly, it disappears and the corridor is fine again -- until you hear a deep whine as something warps right next to you.")
