...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Crumble Away", description=f"Having vanquished the tomb guardians, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: TombGuardiansDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [AssailingTombGuardian(), DefendingTombGuardian()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class TombGuardiansDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Lumbering Figures", description="Patrolling these halls have been a number of golemic figures, creatures of stone glowing with runes and carrying fearsome armaments. At an archway in front of you -- the only path you can see forward into the ruined city -- two of them stand guard: One a thinner figure with a wicked lance and the other bulkier ready to defend with a greatshield as large as it stands tall.")
