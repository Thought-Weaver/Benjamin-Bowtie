...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="No More Pamphlets", description=f"With the cultists vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: CultistsOfAvariceDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [CultistOfAvarice(), CultistOfAvarice(), CultistOfAvarice()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class CultistsOfAvariceDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Who Goes There?", description="Huddled in a corner, you spot a trio of figures wearing long dark robes flecked with gold. Their garments appear to be worn and tattered, made of some kind of spider silk based on the remaining luster. They're turned away from you for the moment, speaking in harsh whispers -- it's certainly not the common tongue, though it almost sounds like a distant dialect of it. One turns their head towards you and gives an immediate order to the other two as they all draw intricate blades.")
