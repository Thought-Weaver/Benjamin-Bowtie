...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Into Mist", description=f"With the echo vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: TimelostEchoDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [TimelostEcho()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class TimelostEchoDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Someone Else?", description="From one of the carved rooms, you can see a figure walk out -- though not quite human. Instead, it appears like some kind of coalescing grey mist with a humanoid top half featuring the remnant of a face. Its lower half is more like a trailing dress of mist that occasionally tries to form itself into a pair of legs. In its grasp, you can see something much clearer: A longsword with markings in a language unknown to you, made of mist itself but still very much corporeal.")
