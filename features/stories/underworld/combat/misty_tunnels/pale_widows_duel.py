...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        ...
        return Embed(title="Widows Widowed", description=f"With the pale widows vanquished, the rest of the caves await.{companion_result_str}")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: PaleWidowDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [PaleWidow(), PaleWidow()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class PaleWidowDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Like Phantoms", description="There are some things feared and prized in the dark: Among them are the phantasmal spiders whose venom is sought after by alchemists and poisoners across the world. Their glassy bodies provide them an otherworldly presence and without a properly prepared antidote, only particularly powerful magic can counteract their poison's effects. One would be bad enough, but skittering from the webs around you now are multiple -- ready to strike.")
