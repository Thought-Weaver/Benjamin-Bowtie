...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: ScuttledarkScorpionDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [ScuttledarkScorpion()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class ScuttledarkScorpionDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="A Large Cavern", description="You come to a region of the caverns where the tunnels open into a larger room. At first you think it would be a safe place to camp for a time -- until you step on the carpet of bones that spreads throughout. From further in the darkness, there's suddenly a rush of movement as something lunges towards you.")
