...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="From Poisoner to Poison", description=f"With the agaric alchemists vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: AgaricAlchemistsDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [AgaricAlchemist(), AgaricAlchemist()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class AgaricAlchemistsDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="A Clear Sign of Danger", description="The bright coloring of anything in nature is a certain sign of its danger and the things in front of you are no exception: Large, moving mushroom people, these ones with red caps speckled white and a shroud of multi-colored vapors. Though they have no eyes, their attention seems to shift towards you all and those same vapors intensity and expand rapidly.")
