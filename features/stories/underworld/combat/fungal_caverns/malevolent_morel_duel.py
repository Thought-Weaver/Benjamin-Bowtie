...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Calm Comes", description=f"With the malevolent morel vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: MalevolentMorelDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [MalevolentMorel()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class MalevolentMorelDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="A Geometric Design", description="The distinctive honeycomb-like pattern and folds are the first thing that strike you as you spot a mushroom creature wandering around the waterfalls and mossy walls. The second thing is the fact that it seems to be attacking other mushroom creatures in a feral way -- or perhaps with an intelligent fury that you'll surely encounter soon.")
