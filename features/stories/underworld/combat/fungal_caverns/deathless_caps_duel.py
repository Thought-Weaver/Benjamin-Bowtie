...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Deathless Indeed", description=f"With the deathless caps vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: DeathlessCapsDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [DeathlessCap(), DeathlessCap()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class DeathlessCapsDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Friend or Foe?", description="Many of the mushroom creatures you've observed in the caves have been far from humanoid, but amid a moss grove you spot two purple mushrooms with arms and legs that grasp onto weapons that look like spears. Their movements are erratic and filled with rage, which you realize suddenly is aimed at you.")
