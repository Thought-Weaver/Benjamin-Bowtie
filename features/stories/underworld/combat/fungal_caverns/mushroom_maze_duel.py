...
class TombsIngressContinueButton(discord.ui.Button):
    def __init__(self):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for player in view.get_players():
            if player.get_dungeon_run().underworld_best_act < UnderworldSection.TombsIngress:
                player.get_dungeon_run().underworld_best_act = UnderworldSection.TombsIngress
...
class TombsIngressIntroView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Tomb's Ingress", description=f"Having made it past the maze of mushrooms that inhabited the caves behind you, the path ahead seems clear again. In fact, as you continue through it, the corridors almost seem to have been carved with tools. There's ornamentation and sigils at certain points which seem to mark the distance to some unknown destination.\n\nThe tunnels soon begin to widen further and further until they almost seem like roads. More often, you find yourselves passing doors and archways that lead into multi-room buildings. The people Yenna mentioned must have once called this place home, but it seems to have been partially abandoned.\n\nPartially, you think, because at times you swear you can still see people moving in the mists -- just outside your vision. And whispering, echoing through the halls of this ancient once-city. Somewhere here is the entity sealed away in its tomb, the thing you have been tasked with destroying once and for all.")

    def _display_initial_buttons(self):
        self.clear_items()
        self.add_item(TombsIngressContinueButton())
...
class TreasureRoomContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: MushroomMazeTreasureRoomView = self.view
...
class MushroomMazeTreasureRoomView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Mushroom Hoard", description=f"Within the maze, you managed to find some items from adventurers not as lucky to have made it through.\n\n{self._treasure_result_str}")
...
class DuelVictoryContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        next_view: MushroomMazeTreasureRoomView = MushroomMazeTreasureRoomView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_dungeon_run())
        initial_info: Embed = next_view.get_initial_embed()
...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Free at Last", description=f"Having made it through the mushroom maze, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: MushroomMazeDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [MushroomMaze()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class MushroomMazeDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Endless Mushrooms", description="The expanse of caverns in this area has only made it less and less clear where you're next supposed to go; meandering side tunnels and dead ends seem to plague you at every turn, not to mention the hordes of mushroom creatures that call this place home.\n\nSomewhere deeper still is the prison for the entity that Yenna warned you about, but to make it there, you'll have to brave the remainder of this mushroom maze.")
