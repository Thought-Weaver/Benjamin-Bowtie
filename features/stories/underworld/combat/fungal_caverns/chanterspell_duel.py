...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Silence at Last", description=f"With the chanterspell vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: ChanterspellDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [Chanterspell(), VolatileIllusion(), VolatileIllusion()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class ChanterspellDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Mushroom Melody", description="From around the corner, there's a disturbance of the otherwise mostly silent mushrooms -- a sound like the chime of a bell or some kind of musical laughter. Emerging from the peripheral is an orangish mushroom creature whose large, flat cap shimmers lightly, as though the bioluminescence of the cave is scattering off its cap. Then, the shimmering solidifies and then there are two more surrounding your party!")
