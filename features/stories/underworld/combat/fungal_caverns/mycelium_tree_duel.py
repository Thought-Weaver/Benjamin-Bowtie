...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Timber?", description=f"With the mycelium tree vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: MyceliumTreeDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [MyceliumTree()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class MyceliumTreeDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="Root and Mushroom", description="The tunnels in this section of the cavern have been wider and more open, filled to the brim with fungi, plants, and running water. Ahead, however, these all seem to converge into a large, organic structure: It looks like an approximation of a tree, towering above with white roots that spread across the ground and leafless branches that pulse with an arcane power.")
