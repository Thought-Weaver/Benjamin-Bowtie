...
class VictoryView(discord.ui.View):
    ...
    def get_initial_embed(self):
        return Embed(title="Scaled Down", description=f"With the salamander vanquished, the rest of the caves await.")
...
class ContinueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view: BlindSalamanderDuelView = self.view
        ...
        duel_view: DuelView = DuelView(view.get_bot(), view.get_database(), view.get_guild_id(), view.get_users(), view.get_players(), [BlindSalamander()], player_victory_post_view=victory_view, player_loss_post_view=defeat_view)
...
class BlindSalamanderDuelView(discord.ui.View):
    def __init__(self, bot: BenjaminBowtieBot, database: dict, guild_id: int, users: List[discord.User], dungeon_run: DungeonRun):
        super().__init__(timeout=None)
    ...
    def get_initial_embed(self):
        return Embed(title="From the Shadows", description="The bioluminescent glow of the mushrooms and moss across the cavern shimmers a beautiful blue that's almost relaxing. But just ahead, you see a section of the fungi that forms something of a bush; the light doesn't quite reach there -- or at least it doesn't seem to. Then the darkness begins to move and you realize it's something else entirely.")
